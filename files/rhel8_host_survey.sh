#!/usr/bin/env bash
# RHEL 8 Host Survey
# Safely collects system details and writes to /var/tmp/host_survey_YYYYmmddHHMMSS
# No changes are made; read/inspect only.
...
cmd_out repos.txt            bash -lc 'dnf repolist -v 2>/dev/null || yum repolist -v 2>/dev/null || true'
...
# Running services (could be large; keep to running)
